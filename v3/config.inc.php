...
$notebook_page = 'ePay';
config_set_notebook_comment($notebook_page, 'epayph plugin configuration');
if (file_exists($rm = dirname(__FILE__)."/readme.txt"))
    config_set_readme($notebook_page, $rm);

add_config_field('payment.epayph.business', 'Merchant ID',
    'text', "your ePay account username",
    $notebook_page, 
    '');

add_config_field('payment.epayph.alt_business', 'Alternate ePay account emails (one per line)',
    'textarea', "",
    $notebook_page, 
    '','','',
    array('default'=>"", 'cols' => 20)
    );
add_config_field('payment.epayph.testing', 'Sandbox testing',
...
add_config_field('payment.epayph.other_account', 'Assign different account to product', 'select', 
...
add_config_field('payment.epayph.dont_verify', 'Disable IPN verification',
...
add_config_field('payment.epayph.lc', 'PayPal Language Code',
...
if (class_exists('payment_epayph')) {
    $pl = & instantiate_plugin('payment', 'epayph');
    $pl->add_config_items($notebook_page);
}

add_config_field('payment.epayph.rewrite_email', 'Set user E-Mail to paypal payer e-mail',
...
add_config_field('payment.epayph.resend_postback', 'Resend Postback',
