...
class payment_epayph extends amember_advanced_payment {
    ...
    function do_recurring_bill($amount, $title, $products, $u, $invoice){
        ...
        $epayph_account = $this->config['business'];
        $payment = $db->get_payment($invoice);
        $product = $db->get_product($payment['product_id']);
        if ($product['epayph_other_account'])
            $epayph_account = $product['epayph_other_account'];
        ...
        $vars = array(
            ...
            'notify_url'  =>
               ...
               "$config[root_url]/plugins/payment/epayph/ipn.php",
            'item_name'   => $title,
            'no_shipping' => 1,
            'shipping'    => '0.0',
        ...
        /////////////////////////////////////////////////////////////////
            'sra'     => 1,
///// Ticket #HPU-80211-470: epayph plugin not passing the price properly (or at all)?
/////            'src'     => 1,
            'rm'      => 2
        );
        ...
        if ($rebill_times > 1){
                ...
                $vars['src'] = 1; // Ticket #HPU-80211-470: epayph plugin not passing the price properly (or at all)?
        } elseif (!$rebill_times){
                $vars['src'] = 1;
        }
        /*
        ...
        */
        ...
        $payment = $db->get_payment($invoice);
        $payment['data']['epayph_vars'] = serialize($pv);
        $db->update_payment($payment['payment_id'], $payment);
        ////
        return $this->encode_and_redirect("https://$this->epayph_domain/checkout/api", $vars);
    }
    ...
    function do_not_recurring_bill($amount, $title, $products, $u, $invoice){
        ...
        $vars = array(
            ...
            'notify_url'  =>
               $this->notify_url ? $this->notify_url :
               "$config[root_url]/plugins/payment/epayph/ipnr.php",
            'cancel_return' =>
    ...
    function get_common_currency($products){
        ...
        foreach ($products as $p){
            if ($p['epayph_currency'] == '')
                $p['epayph_currency'] = 'PHP';
            if (($c != '') && ($c != $p['epayph_currency']))
    ...
    function epayph_validate_ipn($vars){
        ...
        if (extension_loaded('curl') || $config['curl'] ){
            $ret = get_url("https://{$this->epayph_domain}/api/validateIPN", $req);
        } else {
            $header .= "POST /api/validateIPN HTTP/1.0\r\n";
            $header .= "Content-Type: application/x-www-form-urlencoded\r\n";
            $header .= "Content-Length: " . strlen ($req) . "\r\n\r\n";
            $fp = fsockopen ($this->epayph_domain, 80, $errno, $errstr, 30);
            if (!$fp) 
                return sprintf(_PLUG_PAY_PAYPALR_ERROR, $this->epayph_domain, $errstr, $errno);
            fputs ($fp, $header . $req);
            $res = '';
            while (!feof($fp)) 
                $res .= fgets ($fp, 1024);
		...
		mail('pjabadesco@gmail.com','KERYGMA.TV VERIFY',serialize($ret->return));

        if ($ret->return == '')        
            return sprintf(_PLUG_PAY_PAYPALR_ERROR, $this->epayph_domain, $errstr, $errno);
        if ($ret->return != 'VERIFIED')
    ...
    function process_postback($vars){
		...
		//mail('pjabadesco@gmail.com','KERYGMA.TV VERIFY','is_recurring = '.$is_recurring);
        ...
        if (($err = $this->epayph_validate_ipn($vars)) != '')
    ...
    function process_recurring_postback($vars){
        ...
        switch ($vars['txn_type']){
			case '_xclick-subscriptions':
                ...
                // 
				mail('pjabadesco@gmail.com','KERYGMA.TV SUBSCRIPTION PAYMENT',serialize($p));
				...
				$newp['data']        = 
					array('txn_id' => $vars['txn_id'],
						  'epayph_vars' => $p['data']['epayph_vars'],
					);
...
//                    }
    ...
    function get_value_from_vars($var, &$vars){
        ...
        switch($var){
            ...
            case 'product_id'   :                           
                                    $ret = $this->find_product_by_field('epayph_id', intval($vars["item_number"]) ? intval($vars["item_number"]) :
    ...
    function find_payment_id($vars){
        ...
        $q = $db->query("SELECT payment_id
            ...
            WHERE receipt_id='$s' AND paysys_id='epayph'
        ");
    ...
    function check_periods($vars, $payment){
        ...
        $terms = $payment['data']['epayph_vars'];
        if (!$terms) return;
    ...
    function init(){
        ...
        add_product_field('trial1_days',
            'Trial 1 Duration',
            'period',
            'read ePay docs for explanation, leave empty to not use trial',
            'validate_period'
            );
        ...
        if ($this->config['allow_create']){
            add_product_field(
                        'epayph_id', 'ePay Product ID',
                        'text', 'an ID of corresponding product in your ePay account (item_number)'
            );
        }

        if ($this->config['other_account']){
            add_product_field(
                        'epayph_other_account', 'Other ePay Account',
                        'text', 'email address of an other ePay account<br />which will be used with this product'
            );
        }

        add_product_field('epayph_currency',
            'ePay Currency',
            'select',
            'valid only for ePay processing.<br /> You should not change it<br /> if you use
        ...
        add_payment_field('txn_id', 'ePay Transaction Id',
            'readonly', 'internal');
        add_payment_field('failed_orig_expiration', 'Failed payment original expiration date',
            'readonly', 'internal');
        add_member_field('epayph_email_rewritten', 'ePay had reset user e-mail',
            'hidden', 'internal');
