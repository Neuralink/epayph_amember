    ...
    public function validateSource()
    {
        ...
        if (!$this->plugin->getConfig('dont_verify')) {
            ...
            $domain = $this->plugin->getConfig('testing') ? 
                'www.sandbox.paypal.com' : 'epay.ph';
			...
			$ret = json_decode($resp->getBody());

            if ($resp->getStatus() != 200 || $ret->return != 'VERIFIED')
                throw new Am_Exception_Paysystem("Wrong IPN received, epayph [_notify-validate] answers: ".$ret->return.'='.$resp->getStatus());
        }
        /// validate business
